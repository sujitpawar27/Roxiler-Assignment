...
// // Helper function to parse month name to month number
...
const listTransactions = async (req, res) => {
    ...
    console.log('Query:', query); // Log the query being used


    try {
        const total = await Transaction.countDocuments(query);
        console.log('Total:', total); // Log the total count of documents
        ...
        const transactions = await Transaction.find(query)
            ...
            console.log('Transactions:', transactions); // Log the transactions returned

        res.json({ transactions, total, page: parseInt(page), perPage: parseInt(perPage), totalPages });
    } catch (error) {
...
const getStatistics = async (req, res) => {
    try {
        ...
        const stats = await Transaction.aggregate([
            ...
            },
            {
                $facet: {
                    ...
                    soldItems: [{ $group: { _id: null, count: { $sum: { $cond: ['$sold', 1, 0] } } } }],
                    notSoldItems: [{ $group: { _id: null, count: { $sum: { $cond: ['$sold', 0, 1] } } } }]
                }
            }
        ]);
        ...
        res.json({
            ...
            notSoldItems: stats[0].notSoldItems[0] ? stats[0].notSoldItems[0].count : 0
        });
    } catch (error) {
